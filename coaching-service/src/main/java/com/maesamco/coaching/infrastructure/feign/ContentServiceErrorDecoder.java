...
/**
 * 404를 무조건 notFoundErrorCode로 단정하지 않는다(PR #127 심층 재검토, 2026-09-09) —
 * Content Service도 이 프로젝트 공통 에러 포맷(게이트웨이 및 인증 보안 설계 9절)을 쓰고,
 ...
 */
@Slf4j
public class ContentServiceErrorDecoder implements ErrorDecoder {

    /**
     * Content Service가 라우팅 실패(컨트롤러 미배포·경로 오타 등)에 쓰는 공용 코드다.
    ...
    public ContentServiceErrorDecoder(ErrorCode notFoundErrorCode, ErrorCode contractFailureErrorCode) {
    ...
    public Exception decode(String methodKey, Response response) {
        ...
        log.warn("[Coaching] Content Service 404가 리소스 미존재가 아닌 라우팅/계약 실패로 판단됨 "
    ...
    private String readErrorCode(String methodKey, Response response) {
        ...
        try {
            ...
            log.warn("[Coaching] Content Service 404 응답 본문을 표준 에러 포맷으로 파싱하지 못함. methodKey={}",
                    methodKey, e);
            return null;
        }
    }
