...
/**
 * Daily Quiz 완료 보상 반영을 조정하고 동시 갱신 충돌을 재시도합니다.
 *
 * <p>각 시도는 {@link ApplyDailyQuizCompletedRewardTransaction}에서 독립된
 * 트랜잭션으로 실행됩니다. 낙관적 락 충돌이 발생하면 최신 상태를 다시 조회할 수
 * 있도록 새 트랜잭션으로 재시도합니다.</p>
 */
@Service
@RequiredArgsConstructor
public class ApplyDailyQuizCompletedRewardService {

    static final int MAX_ATTEMPTS = 3;

    private final ApplyDailyQuizCompletedRewardTransaction
            rewardTransaction;
    ...
    public boolean apply(
        ...
        throw new IllegalStateException(
                "데일리 퀴즈 완료 보상 재시도 흐름이 비정상적으로 종료되었습니다."
        );
    }
    ...
    private boolean isCommittedDuplicate(
        ...
        return exception.getErrorCode()
                ...
                && xpHistoryRepository.existsBySourceEventId(
                command.quizAttemptId()
        );
    }

    private boolean isRetryableConflict(
            BusinessException exception
    ) {
        return exception.getErrorCode()
