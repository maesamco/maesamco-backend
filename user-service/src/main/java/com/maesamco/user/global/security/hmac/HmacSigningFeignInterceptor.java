 ...
 *
 * ⚠️ basePath를 별도 파라미터로 받는 이유 — Feign은 RequestInterceptor를 적용한 *다음에* Target이
 * {@code @FeignClient(path = ...)}의 prefix를 최종 URL에 합친다(SynchronousMethodHandler.targetRequest():
 * 인터셉터 적용 → target.apply()). 그래서 apply() 시점의 template.url()은 그 prefix가 붙지 않은
 * "/concepts/validate" 같은 값뿐인데, 수신 측 HmacVerificationFilter는 request.getRequestURI()로 실제 도착한
 * 전체 경로("/internal/v1/concepts/validate")를 검증한다. prefix를 서명 문자열에 직접 더해 주지 않으면 정상 요청도
 * 항상 서명 불일치(401)가 난다. coaching-service(이슈 #161)와 content-service(이슈 #163)는 이미 같은 방식으로
 * 고쳤다.
 */
